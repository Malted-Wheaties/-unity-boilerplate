...
public class ForcedPerspectiveV3 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isRayTouchingSomething) crosshair.color = hit.transform.CompareTag("Getable") ? Color.blue : Color.yellow;
        
        if (takenObject != null) pointer.color = Color.red;
        else targetForTakenObjects.position = hit.point;
